...
export default function UsersProfile(props: PropsType) {
    const { loader, userInfo, selectedUser, triggerRefetch, usersData, profileRoute } = props;
            ...
            <div className='container mx-auto px-3 md:px-5'>
                {profileRoute == "post" ? (
                    selectedUser?.posts?.length != 0 ? (
                        selectedUser?.posts?.slice().reverse().map((post: any, i: number) => (
                            <div key={i} className='px-3 py-4'>
                                <div className="header flex justify-between">
                                    <div className='w-auto flex gap-4'>
                                        ...
                                        <div className="w-auto">
                                            <Link href={`/users/${selectedUser?.userName}`} className='text-sky-500 font-bold capitalize text-sm'>{selectedUser?.displayName}</Link>
                                    ...
                                    <div className="w-12 text-end">
                                        {currentUser?._id == post?.postUserId && (
                                            <button onClick={() => {
                                                if (window.confirm("Are You Sure For Delete This Post ?")) setPostForDelete(post?.postUserId);
                                ...
                                <div className="content-post relative">
                                    ...
                                    {post?.content && <ParaGreaph key={i} post={post?.content} />}
                                ...
                                <div className="reacts flex gap-4 py-2">
                                    <button onClick={() => handleReact(post)} className='flex gap-1 items-center'>
                                        ...
                                        <span className='text-xs'>{post?.reacts?.length}</span>
                                    </button>
                                ...
                                <div className="comments">
                                    {
                                        commentLoad ? <div className='text-sm text-center py-4'>Loading...</div> : <CommentsList post={post} />
                                    }
                        ...
                        <p className='text-center py-8 capitalize'>This Profile Has No Posts</p>
                    )
                ) : profileRoute == "about" && <Profile currentUser={selectedUser} />}
            </div>
        </div>
    );
}
