...
export default function MyPost(props: PropsType) {
    const { profileRoute, currentUser, triggerRefetch, usersData } = props;
    ...
    return (
        profileRoute == "post" ? <div className='posts bg-white shadow-md rounded mt-0'>
            ...
            </div>
            {
                ...
                (currentUser?.posts?.length != 0) ? currentUser?.posts?.slice().reverse().map((post: any, i: number) =>
        ...
        </div> : profileRoute == "about" && <Profile currentUser={currentUser} />
    )
}
